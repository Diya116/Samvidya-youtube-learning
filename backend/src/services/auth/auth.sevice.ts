...
import { AuthResponse,UserPayloadPrivate,UserPayloadPublic } from "../../types/auth";
class AuthService{
    ...
    public register=async(req:Request,res:Response,next: NextFunction)=>
    {
     ...
     const hashedPassword:string=await this.getHashedPassword(validatedData.data.password)
     ...
     select:{
      ...
      const accessToken:string=authTokenService.generateAccessToken(user as UserPayloadPrivate);
      ...
      .json({message:"Registed successfully",user:user as UserPayloadPublic})
       }
       catch(err:any)
       {
         return next(err)
    ...
    public authenticate=async  (req: Request, res: Response, next: NextFunction)=>{
      try{
     const accessToken:string|null=req.headers.authorization?.split(" ")[1]||null;
      if (!accessToken || accessToken == "" || accessToken == null || accessToken == "null") {
        throw new Error("Access token not found");
      }
     const userPayload:UserPayloadPrivate=authTokenService.verifyAccessToken(accessToken);
